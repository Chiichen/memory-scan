...
// 暂时设置为 u32 最大值
#[map]
static MAP: HashMap<i64, i64> = HashMap::<i64, i64>::with_max_entries(1000000000, 0);
...
fn increase(ctx: &TracePointContext, address: i64) {
    ...
    if reader.is_some() {
        ...
        let cur = (*pre) + 1;
        if MAP.remove(&address).is_ok() {
            let r = MAP.insert(&address, &cur, 0);
            if r.is_err() {
                error!(ctx, "failed to reinsert a new element into map");
            }
        } else {
            error!(ctx, "failed to remove a previous element from map");
        }
    } else {
        ...
        if r.is_err() {
            error!(
                ctx,
                "failed to insert a new element into map with errno :{}",
                r.unwrap_err()
            );
        }
    }
